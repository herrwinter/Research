...
def merge_training_data():
    ...
    for row in range(len(output_data)):
        arm_count, leg_count = merge_data(output_data[row][0], raw_arm_data, raw_leg_data, arm_count, leg_count)
...
def read_learning_data():
    ...
    raw_y_data = np.loadtxt(base_dir + '/output_data.csv', delimiter=',', dtype=np.float64)
